...
public class PrestadorController {
    ...
    public ResponseEntity<List<PrestadorDTO>> getPrestadoresByServicosNome(@RequestParam String servicosNome) {
        System.out.println("servico_nome:"+servicosNome);
        if (servicosNome == null || servicosNome.trim().isEmpty()) {
            return ResponseEntity.badRequest().body(Collections.emptyList());
        }

        try {
            List<PrestadorDTO> prestadores = prestadorService.findByServicosNome(servicosNome);
            return ResponseEntity.ok(prestadores);
        } catch (Exception e) {
            // Log the exception (e.g., using SLF4J or another logging framework)
            // logger.error("Error while fetching prestadores by servicosNome: {}", servicosNome, e);
